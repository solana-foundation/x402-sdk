...
describe("@x402/svm", () => {
  ...
  describe("stablecoin address helpers", () => {
    it("should return mainnet USDT address", () => {
      expect(getUsdtAddress(SOLANA_MAINNET_CAIP2)).toBe(USDT_MAINNET_ADDRESS);
    ...
    it("should return mainnet CASH address", () => {
      expect(getCashAddress(SOLANA_MAINNET_CAIP2)).toBe(CASH_MAINNET_ADDRESS);
      expect(getStablecoinAddress("CASH", SOLANA_MAINNET_CAIP2)).toBe(CASH_MAINNET_ADDRESS);
    });

    it("should reject USDT on networks without an official mint constant", () => {
      expect(() => getUsdtAddress(SOLANA_DEVNET_CAIP2)).toThrow("No USDT address configured");
    });

    it("should reject CASH on networks without an official mint constant", () => {
      expect(() => getCashAddress(SOLANA_DEVNET_CAIP2)).toThrow("No CASH address configured");
    });
  });
  ...
  describe("ExactSvmScheme (Server)", () => {
    ...
    describe("parsePrice", () => {
      ...
      it("should parse explicit PYUSD and CASH prices", async () => {
        ...
        expect(pyusd.amount).toBe("100000");
        expect(pyusd.asset).toBe(PYUSD_DEVNET_ADDRESS);

        const cash = await server.parsePrice(
          "0.10 CASH",
          "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp",
        );
        expect(cash.amount).toBe("100000");
        expect(cash.asset).toBe(CASH_MAINNET_ADDRESS);
      });
